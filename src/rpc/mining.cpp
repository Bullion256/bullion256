...
if (!chainman.GetParams().IsTestChain()) {
    ...
    if (connman.GetNodeCount(ConnectionDirection::Both) == 0 &&
        active_chainstate.m_chain.Height() <= 1000 &&
        !allow_bootstrap) {
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED,
            PACKAGE_NAME " is not connected!");
    }
}

if (active_chainstate.IsInitialBlockDownload() && !allow_bootstrap) {
    ...
    LogPrintf("DEBUG RPC: height=%d fPreSegWit=%d\n", pindexPrev ? pindexPrev->nHeight : -1, fPreSegWit);
