...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    if (halvings >= 64) {
        // Instead of going to zero like Bitcoin,
        // enforce permanent tail emission of 1 BUL
        return 1 * COIN;
    }

    nSubsidy >>= halvings;

    // If halving reduces subsidy below 1 BUL,
    // switch permanently to tail emission of 1 BUL
    if (nSubsidy < 1 * COIN) {
        return 1 * COIN;
    }
...
{
    // Bullion256 early-stage IBD override
    if (m_chain.Height() > 1000) {
        return false;
    }
