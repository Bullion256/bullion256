...
std::vector<std::shared_ptr<CBlock>> CreateBlockChain(size_t total_height, const CChainParams& params)
{
    ...
    for (size_t height{0}; height < total_height; ++height) {
        ...
        const CAmount subsidy = GetBlockSubsidy(height + 1, params.GetConsensus());
        coinbase_tx.vout[0].nValue = subsidy * 90 / 100; // miner

        coinbase_tx.vout[1].scriptPubKey = P2WSH_OP_TRUE; // treasury placeholder (will be replaced)
        coinbase_tx.vout[1].nValue = subsidy * 10 / 100;  // treasury
